...
export function SettingsClient() {
  ...
  const handleSaveResumeInventory = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || 'Failed to save resume inventory (are you logged in?)')
      }

      toast.success('Resume inventory saved')
    } catch (error) {
      console.error('Error saving resume inventory:', error)
      toast.error(error instanceof Error ? error.message : 'Failed to save resume inventory')
    } finally {
  ...
  const handleImportOptResults = async () => {
    ...
    try {
      ...
      toast.success(`Imported ${data.created} job(s) (skipped ${data.skipped})`)
    } catch (error) {
      console.error('Error importing OPT results:', error)
      toast.error(error instanceof Error ? error.message : 'Failed to import OPT results')
    } finally {
        ...
        <TabsContent value="general" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Default preferences for new applications</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setSettings({ ...settings, autoImportStatus: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setSettings({ ...settings, autoImportPriority: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, statusChangeNotifications: e.target.checked })}
                  className="h-4 w-4"
                />
              </div>
            </CardContent>
        ...
        <TabsContent value="data" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Paste your curated projects/achievements/experience bullet bank as JSON. This is what powers job-specific resume tailoring.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Saving requires login (stored in your `UserPreference` as `resume_inventory_v1`).
                </p>
              </div>

              <div className="flex justify-end">
                <Button onClick={handleSaveResumeInventory} disabled={resumeInventoryLoading || !resumeInventoryJson.trim()}>
                  {resumeInventoryLoading ? 'Saving...' : 'Save Resume Inventory'}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload or paste `OPT_Job_Search_Results.md` to create Applications (status: Saved).
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleOptFileUpload(e.target.files?.[0] ?? null)}
