...
export function ActivityTimeline({ applicationId, application }: ActivityTimelineProps) {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/applications/${applicationId}/activities?${params.toString()}`)
  ...
  const activityTypes = ['all', 'Status Change', 'Interview', 'Email', 'Note', 'Resume']
            ...
            <div className="space-y-6">
              {activities.map((activity, index) => (
                <div key={activity.id} className="relative flex gap-4">
                  <div className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full ${ACTIVITY_COLORS[activity.type] || 'bg-gray-500'} text-white`}>
